...
class DoublyLinkedList:
    ...
    def keep_every_second_element(self):
        ...
        count=0
        node1=self.head
        while node1!=self.tail:
            if count%2:
                self.remove_node(node1)
            node1=node1.next_node
            count+=1
        if count%2:
            node1.previous_node.next_node=None
            self.tail=node1.previous_node
    def remove_node(self,node1):
        
        #print(node.previous_node.next_node.value,node.next_node.value)
        node1.previous_node.next_node=node1.next_node
        #print(node.next_node.previous_node.value,node.previous_node.value)
        node1.next_node.previous_node=node1.previous_node
