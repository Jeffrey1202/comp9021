...
def is_valid_prefix_expression(expression):
    ...
    try:
        temp=list(reversed(expression.split()))
        while temp or len(stack)>1:
            if len(stack)>2 and (type(stack[-1])==int or type(stack[-1])==float) and (type(stack[-2])==int or type(stack[-2])==float):
                x=stack.pop()
                y=stack.pop()
                ope=stack.pop()
                result=ope(y,x)
