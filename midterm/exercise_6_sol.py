...
def f(a, b):
    ...
    for n in range(a,b+1):
        if not judge(str(n),True):
            continue
        for m in range(n+2,b+1,2):
            if not judge(str(m),False):
                continue
            median=(n+m)//2
            if judge(str(median),True) or judge(str(median),False):
                print(n, 'and', m, 'with', median, 'as average')
